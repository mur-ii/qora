...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  Future<void> _onDataRequested(
    ...
    if (state.dataStatus == HomeDataStatus.loading) return;
    emit(state.copyWith(dataStatus: HomeDataStatus.loading));
    try {
      final data = await getHomeData();
      emit(state.copyWith(dataStatus: HomeDataStatus.success, homeData: data));
    } catch (_) {
      emit(state.copyWith(dataStatus: HomeDataStatus.failure));
    }
  }
